...
このプロジェクトでは、**非自明な処理・副作用・仕様に直結するロジック**にはKDocを付け、意図が追えるコードにする。

- ✅ 必須: Repository/UseCase/Factory/DIなどの公開APIで、名前だけでは意図が伝わらないもの
- ✅ 必須: 永続化・時刻/日付計算・状態遷移など仕様に直結する箇所（例: 24時間後削除の起点、日付境界）
- ✅ 必須: 前提条件がある箇所（例: AndroidでContext初期化が必要）
- ❌ 省略可: 自明なDAOのクエリ（例: `getById` の単純SELECT）

KDocは「何をするか」「なぜ必要か」「重要な前提/副作用」を短く書く。

推奨の見出し語（KDoc内）:
- 仕様（内容） / 前提 / 失敗（例外） / 副作用 / 注意

詳細なルールとコピペ用テンプレは [docs/guidelines/coding.md](docs/guidelines/coding.md) の KDoc セクションに従う。
