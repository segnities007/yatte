...
| 概要 | タスクに紐付ける色付きのラベル |
| プロパティ | 名前、カラー |
| 制約 | 名前は必須、色はプリセットから選択 |

### 操作

| 操作 | UseCase | 説明 |
|------|---------|------|
| 作成 | `AddCategoryUseCase` | 新規カテゴリ作成 |
| 編集 | `UpdateCategoryUseCase` | 名前・色の変更 |
| 削除 | `DeleteCategoryUseCase` | カテゴリ削除（タスクの紐付けは解除） |
| 一覧 | `GetAllCategoriesUseCase` | 全カテゴリオブジェクト取得 |
